...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f20x - o2.f20x;
            return cmpX == 0 ? o1.f21y - o2.f21y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!arrayList.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f20x += range.oldListStart;
                snake.f21y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f20x;
                    left.newListEnd = snake.f21y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f20x - 1;
                    left.newListEnd = snake.f21y;
                } else {
                    left.oldListEnd = snake.f20x;
                    left.newListEnd = snake.f21y - 1;
                }
                arrayList.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f20x + snake.size;
                    right.newListStart = snake.f21y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f20x + snake.size + 1;
                    right.newListStart = snake.f21y + snake.size;
                } else {
                    right.oldListStart = snake.f20x + snake.size;
                    right.newListStart = snake.f21y + snake.size + 1;
                }
                arrayList.add(right);
            } else {
        ...
        for (int d = 0; d <= dLimit; d++) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f20x = backward[kOffset + k];
                    outSnake.f21y = outSnake.f20x - k;
            ...
            while (k2 <= d) {
                ...
                if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f20x = backward[kOffset + backwardK];
                    outSnake2.f21y = outSnake2.f20x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f20x != 0 || firstSnake.f21y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f20x + snake.size;
                int endY = snake.f21y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f20x + j;
                    int newItemPos = snake.f21y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f20x + snakeSize;
                int endY = snake.f21y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f20x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f20x + i, 1, this.mCallback.getChangePayload(snake.f20x + i, snake.f21y + i));
                    }
                }
                posOld = snake.f20x;
                posNew = snake.f21y;
