...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f2mX = new float[numPoints];
        this.f3mY = new float[numPoints];
        float[] position = new float[2];
        for (int i = 0; i < numPoints; i++) {
            pathMeasure.getPosTan((((float) i) * pathLength) / ((float) (numPoints - 1)), position, (float[]) null);
            this.f2mX[i] = position[0];
            this.f3mY[i] = position[1];
        }
        if (((double) Math.abs(this.f2mX[0])) > 1.0E-5d || ((double) Math.abs(this.f3mY[0])) > 1.0E-5d || ((double) Math.abs(this.f2mX[numPoints - 1] - 1.0f)) > 1.0E-5d || ((double) Math.abs(this.f3mY[numPoints - 1] - 1.0f)) > 1.0E-5d) {
            throw new IllegalArgumentException("The Path must start at (0,0) and end at (1,1) start: " + this.f2mX[0] + "," + this.f3mY[0] + " end:" + this.f2mX[numPoints - 1] + "," + this.f3mY[numPoints - 1]);
        }
        float prevX = 0.0f;
        int i2 = 0;
        ...
        while (i2 < numPoints) {
            ...
            float x = this.f2mX[componentIndex];
            if (x < prevX) {
    ...
    public float getInterpolation(float t) {
        ...
        int startIndex = 0;
        int endIndex = this.f2mX.length - 1;
        while (endIndex - startIndex > 1) {
            int midIndex = (startIndex + endIndex) / 2;
            if (t < this.f2mX[midIndex]) {
                endIndex = midIndex;
            } else {
                startIndex = midIndex;
            }
        }
        float xRange = this.f2mX[endIndex] - this.f2mX[startIndex];
        if (xRange == 0.0f) {
            return this.f3mY[startIndex];
        }
        float startY = this.f3mY[startIndex];
        return ((this.f3mY[endIndex] - startY) * ((t - this.f2mX[startIndex]) / xRange)) + startY;
    }
}
