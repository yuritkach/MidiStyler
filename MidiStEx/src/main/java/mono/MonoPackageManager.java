    ...
    public static void LoadApplication(Context context, ApplicationInfo applicationInfo, String[] strArr) {
        synchronized (lock) {
            ...
            if (!initialized) {
                ...
                Runtime.init(locale.getLanguage() + "-" + locale.getCountry(), strArr2, getNativeLibraryPath(applicationInfo), new String[]{absolutePath, absolutePath2, nativeLibraryPath}, classLoader, new String[]{new File(externalStorageDirectory, "Android/data/" + context.getPackageName() + "/files/.__override__").getAbsolutePath(), new File(externalStorageDirectory, "../legacy/Android/data/" + context.getPackageName() + "/files/.__override__").getAbsolutePath()}, MonoPackageManager_Resources.Assemblies, context.getPackageName());
